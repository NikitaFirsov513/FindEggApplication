...
void HeightMap::checkSensor(int sensorId, int iter)
{
	...
	//если не прослушивается, то делаем это
	if (!nowSensor->listhen) {

		if (nowValue > entryThreshold)
			return;

		//значение меньше порога обнаружения
		//включаем датчик с разрешением прерывания и значением датчика
	...
	//если прослушивается и прерывание true
	if (nowSensor->listhen && nowSensor->interruptEnabled) {
		...
		if (nowSensor->minValue > nowValue) {
			...
			if (nowValue - nowSensor->minValue > peakDetectionThreshold) {
				//отключаем прерывание для соседних датчиков
	...
	//если прослушивается и прерывание false
	if (nowSensor->listhen && !nowSensor->interruptEnabled || nowValue > entryThreshold) {


		//если упал ниже порога обнаружения, то выключаем
		if (nowValue > entryThreshold) {
